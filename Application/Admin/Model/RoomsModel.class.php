...
class RoomsModel extends Model {
	...
	protected $_validate = array(
        array('room_name', 'require', '名字必须填写！', 1),//1为必须验证
        array('room_price', 'require', '价格必须填写！', 1),//1为必须验证
    );
    	...
    	foreach ($data as $key => $value) {
            ...
            if(++$i%$len == 0){
	            if ($this->create($newdata)) {
	                if ($this->where('place_id='.$place.' AND room_id='.$newid)->data($newdata)->save()) {
	                	unset($newdata);
	                	$ischange = 1;
	                }
	            } else {
    ...
    public function deleteData($place,$key){
    	return $this->where('place_id='.$place.' AND room_id='.$key)->delete();
    }
}